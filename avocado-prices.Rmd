...
To learn more about Hass avocadoes, one can visit https://loveonetoday.com/how-to/identify-hass-avocados/. 

To conduct the analysis, a dataset from Kaggle (found at https://www.kaggle.com/neuromusic/avocado-prices) is used. The dataset gives average Hass avocado prices per unit, as well as other information about the avocados that are sold on a weekly basis.
...
- Are there any relationships between the predictor variables?
...
Looking at the values for 'region' there are overlaps in the regions that are recorded. There are cities/states as well as major regions in the US and the US as a whole. In this analysis, I only focus on major regions in the US, so I divided the cities into regions "east", "west",  and "south". Since I am only looking at one week's worth of data, I want to have multiple observations per region, and grouping cities/states together enabled me to do so. To divide the regions, I stuck with the major regions in the US and grouped the Plains and the West into 'west', the Southeast and Southwest into 'south', and the Northeast into 'east'. Comparing the total volume of these regions with the total volume of the region 'TotalUS' shows that these regions are a subset of the total US, and therefore, there does not seem to be any overlap. There are 15 cities/states per region to give a total of 45 observations for conventional avocados and 45 observations for organic avocados. The processed dataset is shown below. 
...
knitr::kable(avocado_cities[1:5,1:5])
knitr::kable(avocado_cities[1:5,6:10])
```
...
qqnorm(avocado_cities$TotalVolume)
qqline(avocado_cities$TotalVolume)
```
...
knitr::kable(add1(lm.fit,scope=lm.full))

```

The predictor 'type' is added to the model since it minimizes the AIC the most. This makes sense since the average volume output for conventional avocados versus organic avocados differs so much. The summary of the fitted model is then shown.


```{r echo=FALSE}
plot(avocado_cities$type, avocado_cities$TotalVolume, main="Boxplot of Total Volume by Type of Avocado")
...
I considered that there may be some interaction between `4046` and `4225`, which are PLU codes for small/medium and large avocados, respectively. However, adding the iteraction term to the model did not change the multiple R-squared value and caused the adjusted R-squared value to decrease by 0.0018, so it is left out.
...
The large blank chunk in the middle of these graphs are due to the vast differences in organic vs. conventional avocado volumes. 

Looking at the Residuals vs Fitted plot, the residuals seem to follow a linear pattern. The residuals seem equally spaced around the horizontal red line with no discernible pattern. There are a few higher residuals, but they do not seem too worrisome. Later plots may say otherwise. 

Next, looking at the Normal Q-Q plot, the residuals seem to follow the line well, which would indicate that the residuals follow a normal distribution. Observations 1476 and 1164 do seem to be somewhat out of line as opposed to the rest of the points. 
...
Moving forward, to make the most use out of this dataset would be to use all of the observations to make a model of total volume of avocados sold at a point in time. However, doing so would mean that one would have to take into account any correlations due to time and seasonality that exist in the data. As a way to model one week's number of avocodos sold, the model fitted in this report can be useful. To predict a week's avocodo sales would need to take into account other factors regarding this data. 
